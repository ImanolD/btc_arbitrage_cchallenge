...
5. **(~20 s) La tesis, en vivo: EV vs spread.** Abre **Ajustes** (engranaje en la barra superior; muestra el modo activo) y conmuta de **EV** a **spread neto (umbral)**. Verás de inmediato cómo empiezan a dispararse cruces que EV rechazaba por frágiles — la diferencia "bot promedio vs. mesa real" hecha demostración, no solo descrita. Desde el mismo panel se afinan `τ`, costo adverso, EV mínimo y la cadencia de Filo (todo validado y acotado en el servidor).
6. **(~20 s) Pregúntale a Filo.** Abre el chat de **Filo** (botón flotante 🐾) y pregúntale *"¿por qué descartaste ese cruce?"* o *"¿cómo va el P&L?"*: te lo explica en lenguaje llano con los **números reales del motor**. Es la capa de IA **fuera** del hot path — interpreta, nunca decide.
7. **(~15 s) Código.** Todo está tipado de punta a punta (`packages/shared`) y los feeds son **públicos y sin API keys** — el repo corre tal cual. Ver la tabla de abajo para el mapeo criterio↔archivo.
...
| 6 | **Experiencia / UI** | Terminal de trading en vivo (React + shadcn/ui), KPIs, feed bruto-vs-neto, blotter, equity, latencia, triangular, modo demo, **panel de Ajustes en vivo** (EV/spread + parámetros + cadencia de Filo), **chat Filo** (IA grounded fuera del hot path) | `apps/web/src/*`, `apps/server/src/filo/*` |
...
- **EV vs. spread, conmutable en vivo.** El modo de decisión es un parámetro de configuración (`decisionMode`) ajustable desde el dashboard: en `ev` aplica la compuerta de valor esperado; en `spread` (ingenuo) dispara con cualquier neto positivo que pase el riesgo. EV y P(supervivencia) se calculan en ambos modos, así que el jurado puede **conmutar y ver el efecto en el feed al instante** — es la tesis convertida en demostración interactiva, no solo descrita.

**Diferenciador:** combinamos cross-exchange multi-venue **y** triangular multi-venue, y decidimos por **valor esperado** (anticipatorio) en vez de un umbral reactivo — con P(supervivencia) y EV visibles en vivo, y la posibilidad de **demostrar la diferencia en vivo** conmutando a modo spread.
...
**¿El modo de decisión (EV vs. spread) es solo cosmético o cambia el comportamiento real?**
Cambia el comportamiento real del motor. El parámetro `decisionMode` se lee **por referencia** en la ruta de decisión (`arbitrageEngine.ts`): en `ev` una oportunidad debe pasar la compuerta de riesgo **y** superar el EV mínimo; en `spread` basta con un neto positivo que pase el riesgo. EV y P(supervivencia) se siguen calculando en ambos modos (para que la columna no "desaparezca"), pero solo *gobiernan* en modo EV. Por eso, al conmutar a `spread`, empiezan a ejecutarse cruces frágiles que EV descartaba — exactamente el punto que queremos hacer tangible. El cambio viaja por Socket.IO, se **valida y acota** en el servidor (rangos seguros) y se re-emite a todos los clientes (`index.ts → applyConfigPatch`).
